...
class LDPCNetwork(nn.Module):
    ...
    def _apply_ch_weight(self, llr_in, it_idx):
        ...
        if s_val == 2:
            ...
            proto_col = torch.div(v, self.N // self.N_proto, rounding_mode='floor')
    ...
    def _cn_update_SP_par(self, v2c_llr):
        ...
        # Ensure ext_idx is of type torch.LongTensor
        ext_idx = self.edge_to_ext_edge.long()  # Convert to torch.LongTensor
        ...
        x_tanh_3d = torch.where(
            ...
            x_tanh_3d.new_tensor(1.0),  # Neutral value for product
            x_tanh_3d
        )

        # Compute product of extrinsic messages
        prod_ext = torch.prod(x_tanh_3d, dim=2)  # => [B, E]

        # Convert to c2v messages using atanh
    ...
    def _cn_update_MS_par(self, v2c_llr):
        ...
        """
        ext_idx = self.edge_to_ext_edge.long()  # Ensure the index tensor is of type long
        absvals = v2c_llr.abs()
